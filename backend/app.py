...
def init_users_database():
    ...
    # Create some default counsellors and doctors
    default_users = [
...
def setup_fonts():
    """Setup Malayalam fonts for PDF generation"""
    fonts_dir = "fonts"
    ...
    if registered_fonts:
        ...
        print("❌ No Malayalam font found. Please download a Malayalam TTF font to 'fonts/' folder")
        print("📥 Download from: https://fonts.google.com/?query=malayalam")
        app.config['MALAYALAM_FONT'] = 'Helvetica'
        return False

def clean_text_for_pdf(text):
    """Clean text for PDF rendering - remove unwanted characters and fix encoding"""
    if not text:
        return ""
    
    # Remove black squares/dots and other unwanted characters
...
def draw_wrapped_text(can, text, x, y, max_width, font_name, font_size, line_height):
    """Draw text with proper wrapping - FIXED FOR MALAYALAM"""
    if not text:
        return y
    ...
    for paragraph in text.split('\n'):
        ...
        for word in words:
            word_width = can.stringWidth(word + ' ', actual_font, font_size)
...
def create_complete_consent_pdf(patient_info, signature_data, consent_id, doctor_signature_data=None, doctor_signed_by=None):
    """Create a complete PDF consent form with header, content, and signatures - FIXED FOR MALAYALAM"""
    packet = io.BytesIO()
    ...
    # Procedure/Reason Details Section - FIXED: Use Malayalam content directly
    can.setFont("Helvetica-Bold", 14)
    can.drawString(50, y_position, "PROCEDURE/REASON DETAILS:")
    y_position -= 25
    
    # Procedure Details with proper wrapping - FIXED FOR MALAYALAM
    procedure_content = patient_info.get('procedure_details', '')
    if procedure_content:
        # Use Malayalam font if available, otherwise fallback to Helvetica
        font_name = "Malayalam" if malayalam_available else "Helvetica"
        font_size = 10 if malayalam_available else 9
        
        y_position = draw_wrapped_text(can, procedure_content, 50, y_position, 
                                     width - 100, font_name, font_size, 12)
        y_position -= 15
    
    # Ensure declaration and signatures stay on one page
    if y_position < 300:  # If not enough space, start new page
        can.showPage()
        add_header_to_new_page(can, width, height)
    ...
    for point in declaration_points:
        if y_position < 150:  # Ensure enough space for signatures
            can.showPage()
            add_header_to_new_page(can, width, height)
            y_position = height - 150
    ...
    # Signatures Section - ALWAYS ON SAME PAGE AS DECLARATION
    can.setFont("Helvetica-Bold", 14)
    ...
    can.setFont("Helvetica-Bold", 12)
    can.drawString(50, y_position, "Patient/Relative Signature:")
    
    signature_y_position = y_position - 20  # Space before signature
    ...
    if signature_data and signature_data.startswith('data:image'):
        try:
            ...
            can.drawImage(signature_image, 50, signature_y_position - 70, width=new_width, height=new_height, preserveAspectRatio=True, mask='auto')
        except Exception as e:
            print(f"Patient signature error: {e}")
            can.setLineWidth(1)
            can.line(50, signature_y_position - 50, 200, signature_y_position - 50)
    ...
    can.drawString(50, signature_y_position - 75, f"Signed at: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    can.drawString(50, signature_y_position - 90, f"Name: {patient_info.get('signatory_name', patient_info['name'])}")
    can.drawString(50, signature_y_position - 105, f"Relationship: {patient_info.get('signatory_relation', 'Self')}")
    
    # Doctor Signature
    can.setFont("Helvetica-Bold", 12)
    can.drawString(300, y_position, "Doctor's Signature:")
    
    doctor_signature_y_position = y_position - 20  # Space before signature
    
    if doctor_signature_data and doctor_signature_data.startswith('data:image'):
        try:
            doctor_b64 = doctor_signature_data.split(',')[1]
            doctor_bytes = base64.b64decode(doctor_b64)
            temp_img = io.BytesIO(doctor_bytes)
            doctor_image = ImageReader(temp_img)
            
            img_width, img_height = doctor_image.getSize()
            max_width, max_height = 150, 60
            width_ratio = max_width / img_width
            height_ratio = max_height / img_height
            scale_factor = min(width_ratio, height_ratio)
            ...
            can.drawImage(doctor_image, 300, doctor_signature_y_position - 70, width=new_width, height=new_height, preserveAspectRatio=True, mask='auto')
        except Exception as e:
            print(f"Doctor signature error: {e}")
            can.setLineWidth(1)
            can.line(300, doctor_signature_y_position - 50, 450, doctor_signature_y_position - 50)
    else:
        can.setLineWidth(1)
        can.line(300, doctor_signature_y_position - 50, 450, doctor_signature_y_position - 50)
        can.setFont("Helvetica", 10)
        can.drawString(300, doctor_signature_y_position - 70, f"Dr. {patient_info.get('doctor', 'Signature Pending')}")
    
    can.setFont("Helvetica", 9)
    if doctor_signed_by:
        can.drawString(300, doctor_signature_y_position - 85, f"Signed by: Dr. {doctor_signed_by}")
        can.drawString(300, doctor_signature_y_position - 100, f"Signed at: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    else:
        can.drawString(300, doctor_signature_y_position - 85, f"Signed at: Pending")
    
    # Footer
    y_position = signature_y_position - 130
...
def login():
    if request.method == 'POST':
        ...
        if user:
            print(f"✅ User found: {user[1]}")  # Debug log
            print(f"🔑 User active status: {user[6]}")  # Debug log
            print(f"👤 User type: {user[3]}")  # Debug log
            
            if user[6] == 1:  # User exists and is active (is_active is at index 6)
                password_hash = hash_password(password)
                print(f"🔐 Input password hash: {password_hash}")  # Debug log
                print(f"🔐 Stored password hash: {user[2]}")  # Debug log
                
                if user[2] == password_hash:  # Password matches (password_hash is at index 2)
                    session['username'] = username
                    session['user_type'] = user[3]  # user_type from database (index 3)
                    session['full_name'] = user[4]  # full_name from database (index 4)
                    
                    # Update last login
                    update_last_login(username)
                    
                    print(f"✅ Login successful, redirecting to: {user[3]} dashboard")  # Debug log
                ...
                print("❌ User account disabled")  # Debug log
                return render_template('login.html', error="User account is disabled. Please contact administrator.")
        else:
...
def doctor_dashboard():
    ...
    # Get consents pending doctor signature
    conn = sqlite3.connect('consents.db')
    c = conn.cursor()
    c.execute('''SELECT id, patient_name, patient_mrn, consent_required_for, doctor_name, created_at 
                 FROM consents 
                 WHERE doctor_signature IS NULL AND final_pdf IS NOT NULL
...
def save_consent():
    ...
    try:
        ...
        c.execute('''INSERT INTO consents 
                     (patient_name, patient_age, patient_mrn, consent_type, signature_data, counsellor_id, signatory_name, signatory_relation, signatory_mobile, consent_required_for, consent_required_for_ml, procedure_details, procedure_details_ml, doctor_name)
                     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''',
                  (consent_data['patient_name'], consent_data['patient_age'], 
                   consent_data['patient_mrn'], consent_data['consent_type'],
                   signature_data, session['username'],
        ...
        # Create complete PDF
        pdf_packet = create_complete_consent_pdf(patient_info, signature_data, consent_id)
        ...
        return redirect(url_for('success_page', consent_id=consent_id))
    
    except Exception as e:
        print(f"Error saving consent: {e}")
        return f"""
        <h2>Error</h2>
        <p>There was an issue saving the consent. Please try again.</p>
        <p>Error: {str(e)}</p>
        <a href="/dashboard">Back to Dashboard</a>
        """

@app.route('/success/<int:consent_id>')
def success_page(consent_id):
    """Show success page after patient signature"""
    if 'username' not in session:
        return redirect(url_for('login'))
...
def save_doctor_signature(consent_id):
    ...
    try:
        ...
        current_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        c.execute('UPDATE consents SET doctor_signature = ?, doctor_signed_by = ?, doctor_signed_at = ? WHERE id = ?', 
        ...
        print("Creating complete consent PDF with both signatures...")
        
        # Create complete PDF with both signatures
        pdf_packet = create_complete_consent_pdf(patient_info, consent[10], consent_id, doctor_signature_data, session['username'])
        
        # Save the PDF
        final_pdf_path = os.path.join(app.config['GENERATED_FOLDER'], consent[11])
...
def view_consents():
    ...
    optional_columns = [
        ...
        'created_at', 'final_pdf', 'doctor_signature', 'doctor_signed_by', 'doctor_signed_at'
    ]
...
if __name__ == '__main__':
    ...
    print("✅ Database created with ALL required columns")
    print("✅ FIXED: User authentication system")
    print("✅ ADDED: Complete Admin Console")
    print("✅ ADDED: User Management System")
    ...
    print("\n💡 TROUBLESHOOTING:")
    print("If login fails, check the console for debug messages")
    print("Make sure the users.db file is created in your project directory")
